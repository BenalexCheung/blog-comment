...
\begin{center}{\huge \bf Ghostscript 9.21 色彩管理\\} \vspace{0.5in} {\Large Michael J.
Vrhel, Ph.D.\\} {\Large Artifex Software\\} {\Large 7 Mt. Lassen Drive, A-134\\} {\Large Novato, CA 94903, USA\\}
{\Large www.artifex.com\\}
\end{center}
\vspace*{0.5in}
\begin{abstract}
本文档提供了Ghostscript 9.21中有关颜色体系结构的信息。该文档适用于希望通过其输出设备获得准确颜色的用户，希望自定义Ghostscript、通过不同的色彩管理模块来实现更高级别的控制和（或）接口。
\end{abstract}
\begin{center}
\vspace*{0.25in}
修订版本号 1.6
...
在9.0版本中，Ghostscript的颜色体系结构已更新为主要使用ICC \cite{ICC}格式来满足其颜色管理需求。在此版本之前，Ghostscript的色彩架构主要基于PostScript \cite{PS}色彩管理（PCM）。这是因为Ghostscript是在ICC格式之前设计的，甚至可能在考虑数字色彩管理之前。在那个时间点，色彩管理非常具有艺术性，有人调整控件以获得正确的输出颜色。


今天，几乎所有的打印色彩管理都是使用ICC配置文件而不是PCM来执行的。鉴于这一事实以及希望创建更快、更灵活的设计，进行了9.0版本中颜色体系结构的更改。从9.0开始，添加了几个新功能。截至9.21版本，颜色架构的功能包括：

\begin{itemize}
\item 易于使用Ghostscript连接不同的CMM（色彩管理模块）。
\item 所有颜色空间都是根据ICC配置文件定义的。
\item 缓存链接的转换和内部生成的配置文件。
\item 轻松访问ICC配置文件管理器。
\item 轻松指定DeviceGray，DeviceRGB和DeviceCMYK源颜色空间的默认配置文件。
\item 设备可以轻松地传递其ICC配置文件并设置其ICC配置文件。
\item 在多线程环境中高效运行。
\item 使用ICC named color(spots)命名颜色配置文件或使用专有格式。
\item 提供Device-N颜色或者可定制的专色处理的ICC颜色管理。
\item 包括对象类型（例如图像，图形，文本），渲染意图和黑点补偿计算链接变换。
\item 能够使用Ghostscript的默认ICC配置文件覆盖源文档嵌入的ICC配置文件。
\item 易于指定用于图形，图像和文本对象的唯一{\bf source} ICC配置文件。
\item 易于指定用于图形，图像和文本对象的唯一{\bf 目标} ICC配置文件。
\item 易于为图形，图像和文本对象指定不同的渲染意图（感知，色度，饱和度，绝对色度）。
\item 易于为图形，图像和文本对象指定不同的黑点补偿设置。
\item 能够使用PDF输出意图的ICC配置文件。
\item 能够在渲染到分离设备时使用NCLR ICC输出配置文件。
\item 控制仅在渲染到支持黑色墨水的输出设备时强制灰色源颜色为黑色墨水。
\item 能够利用设备链接ICC(device link ICC)配置文件将源颜色直接映射到设备颜色空间。
\item 能够利用设备链接ICC(device link ICC)配置文件从SWOP/Fogra标准色彩空间重定向到特定的设备色彩空间。
\item 能够监控各个页面上是否存在颜色，这对某些打印系统很有用。
\item 能够指定不同的默认透明度混合颜色空间。
\item 能够为某些设备指定后期渲染的ICC配置文件。
\end{itemize}
文档的组织首先提供了架构的高级概述。接下来是各种功能和结构的详细信息，其中包括将其他颜色管理模块与Ghostscript连接所需的信息，以及如何连接专门的颜色处理操作。

\section{整体架构和典型流程}

Figure \ref{fig:ICC_ARCH}提供了构成体系结构的各种组件的图形概览。主要组成部分是：
\begin{itemize}
\item ICC管理器，用于维护各种默认配置文件。
\item 链接缓存，用于存储最近使用的链接变换。
\item 配置文件缓存，用于存储从基于PostScript CIE的色彩空间和CalRGB，CalGray PDF色彩空间创建的内部生成的ICC配置文件。
\item 根文件夹iccprofiles中包含的配置文件，用作输出设备的默认颜色空间和文档中未定义的源颜色。
\item 颜色管理模块（CMM），它是提供和执行转换的引擎（例如，小CMS）。
\item 与设备关联的配置文件，包括依赖于对象类型的配置文件(object type)，校对配置文件(proofing profile)和设备链接配置文件(device link profile)。

\end{itemize}
在典型的流程中，当线程准备好转换数据缓冲区时，它将从链接缓存中请求链接转换(linked transform)。在请求链接时，需要向CMM提供信息，其包括源颜色空间(source)，目标颜色空间(destination)，对象状态（例如文本，图形或图像），黑点补偿设置和渲染类型（例如感知，饱和度，色度）。链接变换提供直接从源颜色空间到目标颜色空间的映射。如果链接缓存中不存在这些设置的链接转换，则将获得CMM的链接转换（假设有足够的内存 - 如果没有足够的内存，则请求的线程将需要等待）。根据CMM，CMM可能会创建一个惰性链接对象（即，当要求它转换数据时创建真实的东西）。在某些时候，链接的转换将返回到请求线程。然后，线程可以使用此映射通过接口调用到外部CMM的数据缓冲区。一旦线程完成了对链接转换的使用，它将通知链接缓存。然后，链接缓存将在由于其他链接请求而需要额外缓存空间时释放链接。
...
\includegraphics*[width=6.5in]{figures/architecture.pdf}
    \end{center}
   \caption{Ghostscript颜色体系结构的图形概述}
    \label{fig:ICC_ARCH}
\end{figure}

\section{PDL颜色定义和ICC配置文件}

为了帮助减少混淆，有必要澄清术语。特别是，需要在ICC配置文件的上下文中定义处理颜色(process color)和设备颜色(device colors)的使用。 PDF\cite{PDF}和PostScript(PS)都区分了印刷色和设备颜色。在PS中，存在从设备颜色到处理颜色的转换（例如，通过UCR/BG）。在ICC工作流程中，颜色直接从输入颜色空间（通常称为源空间）转换为输出颜色空间（通常称为目标空间）。设备的ICC配置文件定义的输出颜色空间是PDF和PS定义为设备的过程颜色空间的映射。换句话说，设备的ICC配置文件定义的“设备颜色空间”是PDF和PS的过程颜色空间。设备的ICC配置文件是从CIE颜色空间到过程颜色空间&从过程颜色空间到CIE颜色空间的映射。

为了更好地理解这一点，创建基于打印的ICC配置文件的方法可能有助于理解。要为设备创建ICC配置文件，将使用其印刷色（例如CMYK）打印图表。该图表使用比色计或分光光度计测量。这提供了从过程颜色到CIELAB值的前向映射。反向映射（从CIELAB到处理颜色）通过通常通过强力搜索和外推方法反转该表来获得。这些映射都打包成ICC格式，从而定义设备“过程颜色”和CIE颜色空间之间的映射。

\section{用法}

有许多命令行选项可用于颜色控制。这些选项也可用作设备参数，因此当在``server-mode''操作中使用Ghostscript时，可以从Ghostscript的命令提示符设置这些选项。

要定义尚未在源文档中以色度方式定义的源颜色，可以调用以下命令行选项：\\
...
在这种情况下，例如，任何设备灰色源颜色将被解释为由ICC配置文件my \ _gray \ _profile.icc定义。如果未设置这些配置文件，则将使用默认ICC配置文件来定义未定义的颜色。这些默认配置文件包含在目录iccprofiles中，名为default \ _gray.icc，默认为\ _rgb.icc，默认为\ _cmyk.icc。配置文件默认值\ _gray.icc用于提供沿中性轴的输出和sRGB线性化。配置文件default \ _rgb.icc是V2 sRGB ICC配置文件，配置文件默认\ _cmyk.icc是SWOP CMYK ICC配置文件。\\
...
可以让Ghostscript使用上面指定的ICC配置文件代替文档中嵌入的ICC配置文件。这是使用\\实现的
